 ...
 ********************************************************************/
 ...
 /* 1/12/20 : Removed new Double, Float and .intValue because they are deprecated
 */
...
class bitrate_manager_state {
	...
	public bitrate_manager_state( vorbis_info vi ) {
		...
		if ( bi.reservoir_bits > 0 ) {
			...
			avg_bitsper = (int) Math.rint(1.*bi.avg_rate*halfsamples/ratesamples);
			min_bitsper = (int) Math.rint(1.*bi.min_rate*halfsamples/ratesamples);
			max_bitsper = (int) Math.rint(1.*bi.max_rate*halfsamples/ratesamples);
			
			avgfloat = PACKETBLOBS/2;    
			
			// not a necessary fix, but one that leads to a more balanced typical initialization
			{
				int desired_fill = (int)(bi.reservoir_bits*bi.reservoir_bias);	// long
