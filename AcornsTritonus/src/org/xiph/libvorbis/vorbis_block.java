 ...
 ********************************************************************/
/*
 * 1/12/20 Remove Double, Integer, Float and the associated parse methods
 */
...
// call to 
	...
	public boolean vorbis_bitrate_addblock() {
		...
		if ( bm.avg_bitsper > 0 ) {
			...
			slew = (float)( Math.rint(choice-bm.avgfloat)/samples*vi.rate );
			if ( slew < -slewlimit )
				slew = -slewlimit;
			if ( slew > slewlimit )
				slew = slewlimit;
