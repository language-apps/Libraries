...
/*
 * 1/12/12 Remove constructors Double, Float, Integer and parseFloat, parseInt - deprecated
 */
...
class mdct_lookup {
	...
	public void mdct_init( int _n ) {
		...
		n = _n;
		log2n = (int)( Math.rint( Math.log( (float)( n ) ) / Math.log( 2.f ) ) );
		...
		// trig lookups...
		for ( i=0; i < n/4; i++ ) {
			trig[i*2] = (float)( Math.cos( (M_PI/n)*(4*i) ) );
			trig[i*2+1] = (float)( -Math.sin( (M_PI/n)*(4*i) ) );
			trig[n2+i*2] = (float)( Math.cos( (M_PI/(2*n))*(2*i+1) ) );
			trig[n2+i*2+1] = (float)( Math.sin( (M_PI/(2*n))*(2*i+1) ) );
		}

		for ( i=0; i < n/8; i++ ) {
			trig[n+i*2] = (float)( Math.cos( (M_PI/n)*(4*i+2) )*.5 );
			trig[n+i*2+1] = (float)( -Math.sin( (M_PI/n)*(4*i+2) )*.5 );
		}
