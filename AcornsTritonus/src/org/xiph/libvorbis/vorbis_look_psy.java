...
//	}
	...
	private float toOC( double z ) {
		return (float)( Math.log(z)*1.442695f-5.965784f );
	}
	...
	private void setup_tone_curves( float[] curveatt_dB, float binHz, int n, float center_boost, float center_decay_rate ) {
		...
		for ( i=0; i<P_BANDS; i++ ) {
			...
			for ( j=0; j<P_LEVELS; j++ ) {
				...
				if ( j < 2 )
					tmp = 2;
				attenuate_curve( workc[i][j], (float)( curveatt_dB[i]+100.0f-(tmp)*10.-P_LEVEL_0 ) );
		...
		for ( i=0; i<P_BANDS; i++ ) {
			...
			bin = (int)( Math.floor(fromOC(i*.5)/binHz) );
			lo_curve = (int)( Math.ceil(toOC(bin*binHz+1)*2) );
			...
			for ( m=0; m<P_LEVELS; m++ ) {
				...
				for (j=0; j<EHMER_MAX; j++ ) {
					...
					bin = (int)( fromOC(j*.125+i*.5-2.)/binHz );
					if ( bin<0 ) {
