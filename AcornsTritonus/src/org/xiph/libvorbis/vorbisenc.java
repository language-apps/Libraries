...
/*
 * 1/12/20 : Removed Double, Float, Integer, Boolean floatValue, intValue because they are deprecated
 */
...
public class vorbisenc {
	...
	public boolean vorbis_encode_setup_managed( vorbis_info _vi, int channels, int rate, int max_bitrate, int nominal_bitrate, int min_bitrate ) {
		...
		float tnominal = (float)( nominal_bitrate );
	...
	public boolean vorbis_encode_setup_init() {		// the final setup call
		...
		if ( hi.bitrate_av > 0 )
			...
			vi.bitrate_nominal = (int)( setting_to_approx_bitrate() );
	...
	private void vorbis_encode_global_psych_setup( float s, vorbis_info_psy_global[] in, float[] x ) {
		...
		ci.psy_g_param = new vorbis_info_psy_global( in[ (int)( x[is] ) ] );
	...
	private void vorbis_encode_residue_setup( int number, int block, vorbis_residue_template res ) {
		...
		// lowpass setup/pointlimit
		{
			...
			if (ci.residue_type[block]==2)
				r.end = (int)((freq/nyq*blocksize*2)/r.grouping+.9) * r.grouping; // round up only if we're well past
			else
				r.end = (int)((freq/nyq*blocksize)/r.grouping+.9) * r.grouping; // round up only if we're well past
		}
	}      
